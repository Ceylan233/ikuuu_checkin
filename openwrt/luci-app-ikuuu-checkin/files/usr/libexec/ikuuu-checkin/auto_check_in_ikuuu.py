...
def infer_mail_provider(address):
    ...
    if domain in ("gmail.com", "googlemail.com"):
        return "gmail"
    if domain in ("yahoo.com", "yahoo.com.cn"):
        return "yahoo"
...
def smtp_settings(address):
    ...
    if provider == "auto":
        provider = infer_mail_provider(address)
    defaults = SMTP_PROVIDERS.get(provider, ("", 465, "ssl"))
    host = os.getenv("SMTP_HOST", "").strip() or defaults[0]
    port = int(os.getenv("SMTP_PORT", "").strip() or defaults[1])
    security = os.getenv("SMTP_SECURITY", "").strip().lower() or defaults[2]
    if not host:
        raise RuntimeError("自定义 SMTP 必须填写 SMTP_HOST")
...
def send_smtp_mail(subject, content):
    ...
    if security == "ssl":
        with smtplib.SMTP_SSL(host, port, timeout=30) as server:
            server.login(smtp_username, mail_pass)
            server.send_message(msg, from_addr=mail_user, to_addrs=recipients)
    else:
...
DATA_DIR = os.path.expanduser(
    os.getenv("IKUUU_DATA_DIR", os.path.dirname(os.path.abspath(__file__)))
)
os.makedirs(DATA_DIR, exist_ok=True)
COOKIE_FILE = os.path.join(DATA_DIR, "ikuuu_cookies.json")
...
def send_email_notification(results, current_domain, captcha_info=None):
    """
    使用已配置的 SMTP 服务发送邮件通知
    """
    title = os.getenv("IKUUU_MAIL_SUBJECT_PREFIX", "iKuuu 签到通知")
...
def send_mail_163(subject, content):
    send_smtp_mail(subject, content)
...
if __name__ == "__main__":
    if "--send-test" in sys.argv:
        send_smtp_mail(
            os.getenv("IKUUU_MAIL_SUBJECT_PREFIX", "iKuuu 签到通知") + "（测试）",
            "SMTP 配置测试成功。",
        )
        print("✅ 测试邮件已发送")
        raise SystemExit(0)
    ...
    # 首先检查是否有域名更新通知
    latest_host = None
    if os.getenv("IKUUU_DISABLE_SELF_UPDATE", "0") != "1":
    ...
    send_email_notification(results, working_domain, captcha_info)
    # ==================== 本地结果输出 ====================
    print("\n📊 签到结果汇总:")
    print("=" * 50)
    success_count = sum(1 for res in results if res['success'])
